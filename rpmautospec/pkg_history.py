...
from .specparser import RPMSpecParser
...
class PkgHistoryProcessor:
    ...
    def _get_rpmverflags(
        ...
        with (
            ...
            # from .specparser import NoRPMSpecParser
            # specparser = NoRPMSpecParser()
            ...
            for spec_candidate in (abridged.name, str(specfile)):
                with tempfile.NamedTemporaryFile(mode="w", prefix="rpmautospec-rpmerr-") as rpmerr:
                    try:
                        epoch_version, info = specparser.query(path, spec_candidate, rpmerr)
                    except Exception:
                        error = True
                        if spec_candidate == str(specfile):
                            rpmerr_out = specparser.get_err()
                    else:
                        error = False
                        rpmerr_out = None
                        break
